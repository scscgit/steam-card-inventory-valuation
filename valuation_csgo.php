...
function get_card_price($card_hash, $target_currency = false) {
  ...
  $listing_url = 'http://steamcommunity.com/market/listings/730/'
...
<p style="text-align:center; color:red">Forked version for Counter-Strike: Global Offensive</p>
<p>The inventory of the given Steam profile has to be public. If yours isn't, you can briefly change it to public, do the valuation, and 

then change it back.</p>
<p>The value is derived from the currently cheapest Community Market listing for any given card. If a card has no listings at all, its 

value is output as &quot;unknown&quot;. The value that is shown does <strong>not</strong> contain the transaction fees, i.e. it is 15% 

lower than what you actually see on the Community Market. It is done this way so that you see what you could potentially gain by selling 

the cards right now, not what the buyer would pay.</p>
<p>There may be slight inaccuracies in the currency conversions due to rounding. For that matter, pricing values may be wrong for any 

number of as-yet unknown reasons. If you see unusual results, please don't take them at face value and double-check before 

trading/selling.</p>
<p>Market listings are cached for 24 hours. Your first request may take a long time to complete, but any subsequent ones on the same day 

should be faster. If it seems like your browser is loading for a very long time, please be patient. If you get a timeout error, please 

refresh -- the program will resume from where it failed. If you have an extremely large amount of cards, you may have to do this several 
...
if($steam_profile != null && $terms_accepted) {
  ...
  $cards_json = file_get_contents('http://steamcommunity.com/profiles/'
                                .$steam_profile.'/inventory/json/730/2');
...
<?php
    ...
    foreach($cards_list as $card) {
      ...
      echo '</td><td>';
      echo '<a href="http://steamcommunity.com/market/listings/730/'
