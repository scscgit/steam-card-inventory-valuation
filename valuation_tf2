...
function get_card_price($card_hash, $target_currency = false) {
  ...
  $listing_url = 'http://steamcommunity.com/market/listings/440/'
...
<p style="text-align:center; color:red">Forked version for Team Fortress 2</p>
...
if($steam_profile != null && $terms_accepted) {
  ...
  $cards_json = file_get_contents('http://steamcommunity.com/profiles/'
                                .$steam_profile.'/inventory/json/440/2');
...
<?php
    ...
    foreach($cards_list as $card) {
      ...
      echo '</td><td>';
      echo '<a href="http://steamcommunity.com/market/listings/440/'
